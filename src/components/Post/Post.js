...
const Post = (props) => {
	...
	return (
		<Card className={classes.root}>
			<CardActionArea>
				<Link to={`/post/${id}`}>
					<CardMedia className={classes.media} image={`https://picsum.photos/id/${200 + id}/200/300`} title="alt text"/>
				</Link>
				<CardContent>
					<Typography variant="h6" component="h5"> {title} </Typography>
					<Typography variant="body2" component="p" color="textSecondary"> {body}</Typography>
				</CardContent>
			</CardActionArea>
			<CardActions>
